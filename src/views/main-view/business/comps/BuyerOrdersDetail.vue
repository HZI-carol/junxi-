<template lang='pug'>
el-dialog.detail-dialog.buyer-orders-detail(title="订单详情" width='900px' :visible.sync="isShow")
  .dialog-content
    .order-info
      .item
        span.label 订单号:
      ...
      .item
        span.label 商家名称:
        span {{curDetail.seller_nickname}}
      .item
        span.label 订单总额:
        span {{curDetail.totalprice}} 元
      .item
        span.label 订单状态:
        span.red(v-if='curDetail.status == 1 && curDetail.isexpires') 已过期
    ...
    el-table(:data="curDetail.orderitems" stripe)
      ...
      el-table-column(prop="" label="金额(元)" width="100")
        template(slot-scope="scope")
          span {{scope.row.price * scope.row.num}}
</template>
