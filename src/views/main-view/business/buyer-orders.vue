    ...
    .content
      ...
      .v-table
        ...
        .order(v-for="item in orderList.list[opts.pageIndex]" :key="item.id")
          .order-head
            .time.omit.order-head-item {{item.addtime | dateFormat}}
            .orderNum.omit.order-head-item 订单号: {{item.productorderno}}
            el-tooltip( :content="item.seller_nickname" placement="top")
              .merchant.omit.order-head-item 商家: {{item.seller_nickname}}
            .total-price.order-head-item 总金额: {{item.totalprice}} 元
            .status.order-head-item
              label 状态:
              span.red(v-if='item.status == 1 && item.isexpires') 已过期
              span.green(v-else) {{item.status_text}}
          .goods-wrap
            .goods-list
              .list(v-for="i in item.orderitems" :key="i.id")
                el-tooltip( :content="i.productname" placement="top")
                  .title.omit.goods-item
                    img.goods-img(:src='i.imgurl')
                    span {{i.productname}}
                .price.goods-item.tac {{i.price}}
                .amount.goods-item.tac {{i.num}}
            .order-options
              el-button(@click='$refs.buyerOrdersDetail.openModal(item)' size='mini' type='primary' plain) 详情
      ...
      BuyerOrdersDetail(ref='buyerOrdersDetail')
</template>
...
export default {
  ...
  methods: {
    ...
    delOrder (item) {
      this.$vgo.open(() => this.$api.delSmartCityMyOrderList(item.id).then(res => {
    ...
    getOrderList () {
      if (!this.orderList.list[this.opts.pageIndex]) {
        ...
        this.$api.getBuyerOrders(this.opts).then(data => {
